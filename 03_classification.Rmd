...
```{r sahd,echo=FALSE, fig.cap="Pair splot of variables for the South African Heart Disease Data. Red points indicate cases and blue points indicate controls.",fig.height=10,fig.width=10}
...
## Subset selection and Elastic net for Logistic Regression

Similar as for linear regression, in the high-dimensional setting where $n$ is small compared to $p$, the maximum likelihood estimator does lead to overfitting and a poor generalisation error. In the context of linear regression we considered constrained estimation, i.e. subset selection and elastic net regularization. It is easy to generalize these to ideas to logistic regression. Subset selection is implemented in `stepAIC` and elastic net regularization in `glmnet`. In the latter case the algorithm optimizes the negative log-likelihood penalized with the elastic net term: 
...
The terms deleted are provided in the next table.
...
## Boosting and Predictive learning

Classification is an important aspect of data mining. There is a variety of methods developed for predictive learning. Popular examples are Support Vector Machines, Random forest, Neural networks and Boosting. We now briefly describe the AdaBoost method which is the most popular boosting algorithm. In the exercises we will then apply it to an example and compare it to the elastic net. 
The key idea behind AdaBoost is to sequentially apply a weak classification algorithm (the "base learner") to weighted data with repeatedly modified weights. In that way a sequence of weak classifiers $G_m, m=1,\ldots,M$ is produced and finally a powerful new classifier is obtained by combining through a majority vote. More specifically AdaBoost proceeds as follows:
...
3. Output combined predictor $G(X)={\rm sign}\left[\sum_{m=1}^M \alpha_m G_m(X)\right]$.
...
AdaBoost with classification trees as the "base learner" is one of the best "off-the-shelf" classifier in data mining. Data mining problems also deal with a large number of variables, however, typically not only $p$ is large but also the sample size $n$. In the classical high-dimensional setting with $p>>n$ the elastic net is an extremely powerful method. 