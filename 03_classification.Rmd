...
We so far focused on the linear regression model. We will now extend to the setting where the response is binary or time-to-event. The former is also referred to as classification and the latter as survival analysis.
...
We turn our attention to survival analysis which deals with so-called time-to-event endpoints. We start with a very brief introduction to survival analysis and the cox proportional hazards model. Then we turn to the high-dimensional setting and discuss elastic net regularization for the cox regression. Finally, we introduce the time-dependent Brier score to compare prediction performance in the survival setting. 
...
The hazard function $h(t)$, or the instantaneous rate at which events occur, is defined as

\[h(t)=\lim_{dt\rightarrow 0}\frac{P(t\leq T < t+dt|T\geq t)}{dt}=-S'(t)/S(t).\]

The Kaplan-Meier method is the most common way to estimate survival times and probabilities. The Cox proportional hazards model is commonly used for the study of the relationship between predictor variables and survival time. The Cox model assumes a semi-parametric form for the hazard
...
We can use the `summary` function to obtain more specific information of the estimated survival quantities, e.g. the probability of surviving beyond 10 years is obtain as

```{r, eval=FALSE}
summary(survfit(Surv(time, status) ~ 1, data = dat), times = 10)
```

We are interested to find the genes which are most predictive for survival. (Note that we only consider $p=500$ genes with largest variance.) We use glmnet to perform L1-penalized cox regression.
...
To identify the optimal lambda we use cross-validation and take Harrel's concordance measure as the loss function.
...
The tuning parameter $\lambda_{\rm{opt}}$ which maximizes the C-index is `r round(cv.coxnet$lambda.min,2)`. The next graphic shows the magnitude of the non-zero coefficients (note that we standardized the input covariates).
...
Finally we use the `pec` package to calculate the Brier score on the train and test data.
