...
We so far focused on the linear regression model. We will now extend our previous considerations to the setting of classification.

Consider the binary situation where response $G$ takes values $0$ and $1$, and, the aim is to do prediction based based on covariates $X=(X_1,\ldots,X_p)$. We model the probability of success $$p(x;\beta)=P(G=1|X=x;\beta)$$ using the logistic regression approach with logit link function
