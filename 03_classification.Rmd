...
Therefore in practise we observe the survival time $Y_i$ (which equals the event time or the censoring time whichever occurs earlier) and the event indicator $\delta_i$ ($\delta_i=1$ in case of event $=0$ in case of censoring).
...
$h_0(t)$ is the baseline hazard and $\beta$ are the regression coefficients. In the classic setting $n>p$, inference is made via the partial likelihood. Similar as before we can use subset selection or we can use glmnet which penalizes the negative log of the partial likelihood.
...
The data consists of gene expression data for $p=7399$ genes measured on $n=240$ patients, as well as censored survival times, for these patients. We start by reading the data and plotting the estimated surival curve.
...
fit.surv <- survfit(Surv(time, status) ~ 1, data = data.frame(y))
ggsurvplot(fit.surv)
```
...
We are interested to find the genes most predictive for surival. We start by filtering 
for the $p=500$ genes with highest variance.

```{r}
top.genes <- order(apply(x,2,var),decreasing=TRUE)[1:500]
x <- x[,top.genes]
dat <- data.frame(cbind(y,x))
```

We now use glmnet to perform L1-penalized cox regression.

```{r}
set.seed(1)
fit.coxnet <- glmnet(x, y, family = "cox")
plot(fit.coxnet,xvar="lambda")
```


To identify the optimal lambda we use cross-validation and choose Harrel's concordance measure as loss function.

```{r}
cv.coxnet <- cv.glmnet(x,y,family="cox",type.measure="C")
plot(cv.coxnet)
```
The optimal lambda is

```{r}
cv.coxnet$lambda.1se

betahat <- coef(fit.coxnet,s=cv.coxnet$lambda.1se)
Shat <- rownames(betahat)[which(betahat != 0)]
```

We run classical cox model
```{r}
fm <- as.formula(paste0("Surv(time, status) ~ ",paste(Shat,collapse = "+")))
fitshat <- coxph(fm,data=dat,x=TRUE,y=TRUE)
```
...
fm <- Surv(time, status) ~ .
fitcforest <- pec::pecCforest(fm, data =dat, 
...
```{r}
library(pec)
models <- list("cox.shat"=fitshat,"cforest"=fitcforest)
fitpec <- pec::pec(
  object=models, 
  data = dat, 
