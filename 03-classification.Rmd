...
Classification is a frequent task in data mining and besides logistic regression there is a variety of other methods developed for this task. One such approach are so-called Classification trees. They learn a binary tree where leaves represent class labels and branches represent conjunctions of features that lead to those class labels. The package `rpart` can be used to learn a classification tree.
...
```
The algorithm start by growing a typically too large tree which overfits the data. The next step is to "prune" the tree to obtain a good tradeoff between goodness of fit and complexity. `rpart` provides cost-complexity prunings based on cross-validation.
