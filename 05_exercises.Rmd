...
Hint: take the derivative of the loss function $\ell_{\rm Ridge}(\beta|\bf Y,\bf X)=\rm{RSS}(\beta)+\lambda\|\beta\|^2$ and set equal to zero. 
...
3. Use Subset selection, Elastic net regularization and AdaBoost for classification. (Hint: use the function `gbm` with argument `distribution` set to "adaboost").
...
We can repeat the same analysis using the `train` function from the caret package.
...
Next we investigate boosting. We can use the `gbm` function
