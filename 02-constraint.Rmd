...
And the regression coefficients of the optimal model can be retrieved
...
```{r ridgegeom,echo=FALSE,out.width="70%",fig.cap="Figure displaying the Ridge geometry."}
...
Both formulations are equivalent in the sense that there is a one-to-one relationship between $c$ and $\lambda$. We will use more often the latter "penalisation" formulation. The parameter $\lambda$ is the amount of penalisation. Note that for "no penalization", $\lambda=0$, Ridge regression coincides with OLS. Increasing $\lambda$ has the effect of shinking the regression coefficients to zero. 

The Ridge optimization problem has the closed form solution (see Section \@ref(exercises))

\begin{align*}
\hat{\beta}^{\rm Ridge}_{\lambda}&=(\bf X^T \bf X+\lambda \bf I)^{-1}\bf X^T \bf y.
\end{align*}

Note that for $\lambda>0$ the matrix ${\bf X}^T {\bf X}+\lambda \bf I$ has always full rank and therefore Ridge regression is well defined in the high-dimensional context (in contrast to OLS).
...
We proceed as explained in Section \@ref(statistical-learning). The tuning parameters is $\lambda$ and we select a grid $0<\lambda_1<\lambda_2<\ldots<\lambda_K<\infty$. We choose the optimal $\lambda_{\rm opt}$ by either re-sampling or information criteria. In `glmnet` we use cross-validation
...
We choose an optimal tuning parameter $\lambda_{\rm opt}$ by cross-validation.
...
The Lasso has the lowest RMSE. We plot the regression coefficients for all 3 methods.
...
We point out that the same analysis can be conducted with the powerful `caret` package. The code to do so is provided next.
