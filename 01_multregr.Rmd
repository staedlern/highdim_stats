...
In this chapter we will review multivariate regression and in particular the *Ordinary Least Squares* (OLS) estimator. We will further introduce the topic of *overfitting* and *high-dimensionality*.

## Ordinary Least Squares

In this section we briefly review the OLS estimator. Given a vector of inputs $X=(X_1,X_2,\ldots,X_p)$, we predict the output $Y$ via the model:
...
How do we fit the linear model to a set of training data? In multivariate regression we typically use *ordinary least squares* (OLS). In this approach, we pick the coefficient $\beta$ to minimize the residual sum of squares
...
There are two important geometric interpretations of the OLS estimator. Figure \@ref(fig:olsgeom2) shows a "row representation". Here, the $n$ data points $(y_i,x_{i2},\ldots,x_{ip})$ randomly spread around a $(p-1)-$dimensional hyperplane in a $p-$dimensional space. The random spread only occurs parallel to the y-axis. 

```{r olsgeom2,echo=FALSE, fig.cap="Respresentation of the $n$ data points randomly spreading around a 2-dimensional hyperplane."}
knitr::include_graphics("ols_geom2.JPG")
```

Figure \@ref(fig:olsgeom1) shows a "column representation". Here, the vector $\bf y$ of observation is a single point in the $n-$dimensional space ${\bf R}^n$. The fitted values $\hat \bf y$ are the orthogonal projection onto the $p-$dimensional subspace of ${\bf R}^n$ spanned by the vectors ${\bf x_1},\ldots,{\bf x_p}$.

```{r olsgeom1,echo=FALSE, fig.cap="Representation of the fitted values as an orthogonal projection onto the 2-dimensional subspace spanned by the covariates."}
