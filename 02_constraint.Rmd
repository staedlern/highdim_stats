...
It is easy to show that this is equivent to OLS regression based covariates in the subset $S$, i.e.
...
In `R` we can use `regsubsets` from the `leaps` package or `stepAIC` from the `MASS` package to perform subset selection. For example to perform forward stepwise selection based on AIC
...
The selection process can be summarized by 

```{r}
kable(as.data.frame(fit.fw$anova),digits=3)
```

And the regression coefficients of the optimal model are given by
...
An alternative is *Ridge regression*, which constrains/regularizes the optimization problem by shrinking regression coefficients towards zero. This discourages complex models because
...
The geometry of the optimization problem is illustrated in Figure @\ref{fig:ridgegeom}. It shows the levels sets of the ${\rm RSS}(\beta)$ which are ellipsoids and centered around zero the circular ridge
parameter constraint parametrized by some $c > 0$. The Ridge estimator
...
```{r ridgegeom,echo=FALSE,out.width="70%",fig.cap="Figure is displaying the Ridge geometry."}
...
This can be verified by taking the derivative of the loss function $\ell_{\rm Ridge}(\beta|\bf Y,\bf X)=\rm{RSS}(\beta)+\lambda\|\beta\|^2$ and set equal to zero. Note that for $\lambda>0$ the matrix ${\bf X}^T {\bf X}+\lambda \bf I$ has always full rank and therefore Ridge regression is well defined in the high-dimensional context (in contrast to OLS).


### Choice of penalty parameter

We proceed as explained in Section \@ref(statistical-learning). The tuning parameters is $\lambda$ and we select a grid $0<\lambda_1<\lambda_2<\ldots<\lambda_K<\infty$ and choose the optimal $\lambda_{\rm opt}$ by either re-sampling or information criteria. In `glmnet` we use cross-validation
...
Although Ridge regression involves all $p$ covariate the *effective degrees of freedom* is smaller than $p$ as we have imposed constraints through the penalty. In the book @elements it is shown that the effective degrees of freedom for Ridge regression, $\nu^{\rm ridge}_{\lambda}$, is given by

\[\nu^{\rm ridge}_{\lambda}=\sum_{j=1}^{p}\frac{d_j^2}{d_j^2+\lambda},\]
where $d_1,\ldots,d_p$ are the singular values of $\bf X$. In our example we have
...
The residual degrees of freedom is $n-\nu^{\rm ridge}_{\lambda}$=`r round(nrow(xtrain)-df_lambdaopt,2)`. Note that this is inline with the "rule of thumb" we mentioned in Section \@ref(overfitting).

### Shrinkage property

In Section \@ref(collinearity) we discussed that in presence of collinearity the OLS estimator becomes unstable (high variance) and that this becomes a challenge in high-dimensions. A nice property of Ridge regression is that it counteracts this by shrinking low-variance components more than high-variance components. 

The OLS vector of fitted values ${\bf \hat y}^{\rm OLS}$ is the orthogonal projection of ${\bf y}$ onto the column space of $\bf X$. In terms of the normalized principle components we have

\[{\bf\hat y}^{\rm OLS}=\sum_{j=1}^{p}{\bf u}_j {\bf u}_j^T {\bf y}.\]

Similarly we can represent the Ridge projection as

\[{\bf\hat y}^{Ridge}=\sum_{j=1}^{p}{\bf u}_j \frac{d_j^2}{d_j^2+\lambda}{\bf u}_j^T {\bf y}.\]

This shows that the shrinkage $\frac{d_j^2}{d_j^2+\lambda}$ is largest in the direction of the last principle component, which in return is the direction in the column space of $\bf X$ with largest variance (see Figure \@ref{fig:principlecomponent}.

```{r principlecomponent,echo=FALSE,out.width="70%",fig.cap="Figure displaying principle components of 2-dimensional input data."}
knitr::include_graphics("principlecomponents.JPG")
```

### Bayesian interpretation

Ridge regression has a nice interpretation as the maximum a posteriori (MAP) estimate of a hierarchical bayesian model: 
...
Ridge regression and high-dimensionality play a role in many subfields of statistics. We illustrate this with the example of univariate non-parametric regression.
...
One approach to find a non-parametric approximation are so-called splines. We assume the unknown function $f$ is represented by a spline function which can be expressed by a set of basis functions 

$$ f(X)=\sum_{m=1}^{p}\beta_m B_m(X).$$
For example for a cubic spline with $K$ fixed knots and fixed polynomial degree $d=3$ ("cubic") we have $p=K+d+1$ and the $B_m(x)$'s are a B-spline basis (one could also use the truncated-power basis). The coefficients $\beta_m$ are estimated using OLS. Although we have only one single variable $X$ the design matrix consists of $p=K+d+1$ features and the estimation problem becomes quickly "high-dimensional".  
...
We fit a cubic spline and smoothing splines to our simulation example.

```{r}
fit.csp <- lm(y~bs(x,df=10)) # cubic spline with p=10 degrees of freedom
...
The cubic spline with $p=10$ (in violet) fits the data well. The smoothing spline with $\nu=30$ (in green) leads to overfitting. The smoothing spline with $\nu=10$ with $\lambda$ obtained by cross-validation (in red) are both good approximation of the truth. The corresponding effective degrees of freedom of the cross-validation solution can be retrieved from the model fit.
...
To identify an optimal model we choose an optimal tuning parameter $\lambda_{\rm opt}$ by cross-validation.
...
The optimal Lasso coefficient can be extracted using the `coef` function.
...
In our example the set of selected variables can be obtained as follows.
...
We start by splitting the data into train and test data.
...
We continue by fitting Ridge regression and the trace and cross-validation plots.
...
The Lasso has the lowest RMSE. Finally we plot the regression coefficients for all 3 methods.
...
We finally show the code how we can use the powerful `caret` package to perform the same analysis.
