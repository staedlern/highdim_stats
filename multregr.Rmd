...
We simulate training data with $p=9$ covariates $X_{i1},\ldots,X_{ip}$ as i.i.d $N(0,1)$ and outcome $Y$ which depends only on the first covariate according to $Y_i\sim \beta_1 X_{i1}+\epsilon_i$, $\beta_1$, $\epsilon_i\sim N(0,0.5)$. The size of the training data is $n=10$.
...
In this example the model fits the data perfectly. The residual degrees of freedom to estimate the error is n-p=`r n-p`. A rule of thumb to avoid overfitting is that the residual degrees of freedom should be at least $10$.
...
In the case of super-collinearity the OLS is not well-defined as the rank of the design matrix is 0 and therefore the matrix $\bf X^T \bf X$ is singular and therefore not invertible. In our example, if we include x4 into the design matrix, the design matrix has not full rank
...
The most traditional approach to impose constraints is subset selection. In this approach we retrain only a subset of the variables, and eliminate the rest from the model. OLS is used to estimate the coefficients of the inputs that are retained. More formally, given a subset $S\subset\{1,\ldots,p\}$ we solve the optimization problem


\begin{align*}
\hat{\beta}_{S}&=\rm{argmin}_{\beta_j=0\;\rm{for\; all}\;j\notin S}\rm{RSS}(\beta)\\
&=(\bf X_S^T \bf X_S)^{-1}\bf X_S^T \bf y.
\end{align*}
...
\begin{align*}
\hat{\beta}^{\rm Ridge}_{\lambda}&=\rm{argmin}_{\|\beta\|^2\leq c}\rm{RSS}(\beta).
\end{align*}
...
In our example the residual degrees of freedom is $n-DF^{\rm Ridge}(\lambda_{\rm opt})=$`r round(nrow(xtrain)-df_lambdaopt,2)` which lies within our "rule of thumb" of minimum of $5-10$ residual degrees of freedom to avoid overfitting.
...
2. Riboflavin data set from hdi package

3. Calculate the Ridge and the Lasso solution for orthogonal covariates.

4. Write down the log-likelihood of the model
