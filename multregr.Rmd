...
\begin{align*}
\hat{\beta}^{\rm Lasso}_{\lambda}&=\rm{argmin}-\frac{1}{n}\rm{RSS}(\beta)+\lambda\|\beta\|_1.
\end{align*}
...
```{r}
cv.lasso.glmnet <-cv.glmnet(x=x_train,y=y_train,alpha=1) 
plot(cv.lasso.glmnet)
cv.lasso.glmnet$lambda.1se
```

The optimal Lasso regression coefficient can be optained as

```{r}
beta.lasso <- coef(fit.lasso.glmnet, s = cv.lasso.glmnet$lambda.1se)
names(beta.lasso) <- colnames(x_train)
beta.lasso
```
...
x2 <- cbind(x,x^2)
colnames(x2)[11:20] <- paste0(colnames(x2)[1:10],"2")
xint <- model.matrix(~0+.^2,data=data.frame(x))
xx <- cbind(x2,xint)
head(diabetes$x2[,""])
head(x[,"age"]^2)
age2 <- dat$age^2
age2c <- age2-mean(age2)
head(age2c/sqrt(sum(age2c^2)))

head((dat$age^2-mean(dat$age^2))/sqrt(sum((dat$age^2-mean(dat$age^2))^2)))
...
We finally show how we can use the caret package to perform Forward, Ridge and Lasso regression. 
With the caret package we can also easily compare the 3 methods.

```{r, echo=FALSE}
tc <- trainControl(method = "cv", number = 10)
lw <- ~.
up <- formula(paste("~", paste(colnames(x2s), collapse=" + ") ))
fit.fw.caret <- train(x=x2s,
                      y=y,
                      method = "lmStepAIC", 
                      scope = list(upper=up),
                      direction ="forward",
...
```{r}
lambda.grid <- fit.ridge$lambda
fit.ridge.caret<-train(x=x2s,
                       y=y, 
                       method = "glmnet",
                       tuneGrid = expand.grid(alpha = 0,lambda=lambda.grid),
                       trControl = tc
...
# Model coefficients
coef(fit.ridge.caret$finalModel,fit.ridge.caret$bestTune$lambda)
# Make predictions
yhat <- fit.ridge.caret %>% predict(dat)
```

```{r}
lambda.grid <- fit.lasso$lambda
fit.lasso.caret<-train(x=x2s,
                       y=y, 
                       method = "glmnet",
                       tuneGrid = expand.grid(alpha = 1,lambda=lambda.grid),
                       trControl = tc
...
# Model coefficients
coef(fit.lasso.caret$finalModel,fit.lasso.caret$bestTune$lambda)
# Make predictions
yhat <- fit.lasso.caret %>% predict(dat)
```
