...
2. Create a scatterplot matrix for all variables of the prostate cancer data. Use `pairs` or `ggpairs`.
3. Standardize the predictors to have unit variance and divide the data into training and test sets (see variable `train`). How many test and training samples do we have?
4. Run a univariate regression model with `lcavol' as covariate. Study the `summary` output. 

   - What do you interpret the regression coefficients for `lcavol`?
   - Explain the  *Multiple R-squared*? 
   - What is the *Residual standard error*?
   - Generate a scatter plot of `lpsa` on `lcavol` and add the regression line with confidence band (use `geom_smooth`, `method="lm"`). 
   - Draw the Tukey Anscombe plot and the QQ plot (`?plot.lm`). What are these two plots telling us?
   
5. Run a multiple regression model using all covariates. Study the `summary` output. 

   - What changes in the interpretation of the coefficient for `lcavol`? 
   - What do you conclude from the *Multiple R-squared* and the *Residual standard error*?
   - Create the Tukey Anscombe plot and a QQ plot (`?plot.lm`).
   
6. Run a multiple regression model including all covariates but `lacvol`. Proceed as in exercise 5. 
...
We generate a histogram of `lpsa`.
```{r}
hist(dat$lpsa)
```
...
nrow(dtest)
```
We fit a univariate regression model.
```{r}
...
We run the multiple regression model with all covariates. We print the `summary` and create the TA and QQ plots.
```{r}
...
We run the multiple regression model without covariate `lcavol`. We print the `summary` and create the TA and QQ plots.
```{r}
...
We compare the 3 models with the `anova` function.

```{r}
anova(fit1,fit2,fit3)
```

## Generalization error and validation of a prognostic model

1. Calculated the RMSE on the training data. Which model will perform best on future data?
2. Use the test data and make scatter plots of the observed vs predicted outcomes (one scatter plot per model). This plot is also called a "calibration plot". Use `ggplot` and add a regression line (`geom_smooth`) and the "y=x" (`geom_abline`) line to the graph.
...
5. Finally calculate the generalization error, i.e., the RMSE on the test data.
...
Boxplot of predcited vs observed. 
...
In this exercise we explore linear regression using the following simulated data.
...
1. Fit the following 3 models on the training data: a univariate regression model with only X1, a multiple regression model with X1-X4 as covariates and a multiple regression model including all covariates. Explore the fitted models.
2. Repeat your calculations from exercise 2. What do you conclude?
...
```
We draw calibration plots for the 3 models. Only model 2 is well calibrated.
...
4. Show that $\|a\|^2=a^Ta$.
5. Use the result in 4. and show that $\bf RSS(\beta)=\bf y^Ty-2 y X \beta +\beta^T X^T X \beta$. 
6. Invoke the result obtained in 4. and show that the derivative of $\bf RSS(\beta)$ equals $\bf -2X^Ty+2X^TX\beta$ (Hint: review the "Identities" section of [Wikipedia](https://en.wikipedia.org/wiki/Matrix_calculus)).
...
Explore the prostate cancer data set described in the book @elements. The data is available from [github](https://github.com/staedlern/highdim_stats/tree/main/data). 
The variables are log cancer volume (`lcavol`), log prostate weight (`lweight`), `age`, log of the amount of benign prostatic hyperplasia (`lbph`), seminal vesical invasion (`svi`), log of capsular penetration (`lcp`), Gleason score (`gleason`), and the percent of Gleason scores 4 or 5 (`pgg45`). Further there is a variable `train` indicating training and test set.
