...
Explore the prostate cancer data set described in the book @elements. The data is available at [github](https://github.com/staedlern/highdim_stats/tree/main/data). 
...
1. Read the prostate cancer data set and make histogram for all variables.
2. Create a scatterplot matrix of the prostate cancer data.
3. Standardize the predictors to have unit variance and create divide the data into training and test sets. How many test/training samples do we have?
4. Run a univariate regression model with `lcavol' as covariate. Study the `summary`. What is the interpretation of the coefficient? Produce a scatter plot of `lpsa` on `lcavol` and add the regression line with confidence band. Make a Tukey Anscombe plot and a QQ plot.
5. Run a multiple regression model with using all covariates. Study the `summary`. What is the interpretation of the coefficient? Make a Tukey Anscombe plot and a QQ plot.
6. Calculate the RSS for both models. Which model is better? 
2. Split the data into training and test set. How many training and test samples do we have?
...
Standardize predictors and create training and test data.
```{r}
...
```
Tukey Anscombe plot. The plot indicates that the residuals are independent and have mean 0 (no systemetic pattern).

```{r}
plot(fit1,which=1) # Tukey Anscombe plot
```

The QQ plot is used to check the normality assumption of the residuals. The residuals show slight tendency to be left-tailed.
```{r}
...
The QQ plot. This plot can be used to checks the normality assumption of the residuals. In this case it 
...
```
Make a scatter plot of fitted vs observed.

```{r}
plot(predict(fit1),dtrain$lpsa)
RMSE(dtrain$lpsa,predict(fit1))
plot(predict(fit2),dtrain$lpsa)
RMSE(dtrain$lpsa,predict(fit2))
summary(lm(dtrain$lpsa~predict(fit1)))
summary(lm(dtrain$lpsa~predict(fit2)))
```
...
```{r}
dd <- data.frame(pred=predict(fit1,newdata=dtest),obs=dtest$lpsa)
dd%>%
  ...
  theme_bw()
summary(lm(obs~pred,data=dd))
```
Calibration plot for model 2.
```{r}
dd <- data.frame(pred=predict(fit2,newdata=dtest),obs=dtest$lpsa)
dd%>%
  ggplot(.,aes(x=pred,y=obs))+
  geom_point()+
  geom_smooth(se=FALSE)+
