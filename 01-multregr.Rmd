...
In this simulation exercise we illustrated the problem of overfitting. In high-dimensional settings ($p$ large relative to $n$) we quickly face models which fit the data well, but in fact are poor approximations of the true underlying data generation mechanism. 
...
In practice we do not know the underlying truth. How do we know when a model is overfitting? The following rule of thumb (the "10:1" rule) can be helpful: 

> In order to avoid overfitting the number of predictors $p$ should be less than $n/10$.

A more formal way to answer the question is by examining the so-called *Generalization Error*. This will be discussed next. 
...
The models with $p=1$ and $4$ achieve a good error close to the "irreducible" $\sigma_{\epsilon}$. On the other hand the predictions obtained with $p=8$ and $15$ are very poor (RMSE 6 to 8-fold larger).
