...
```{r}
fit1 <- lm(y~0+X1,data=dtrain)
```

Next, we plot the fitted values against the values of the first covariate and we add the reference line representing the “true” relationship. The fitted values (red triangles) perfectly match the data (black dots). 
...
t.d%>%
  ...
  xlab("X1")+ylab("Y")
```
Next, we add two "noise" covariates to the model.

```{r}
fit3 <- lm(y~X1+X2+X3,data=dtrain)
```

The fitted values start to deviate from the truth. 
```{r, echo=FALSE}
t.d <-data.frame(x=xtrain[,1],y=ytrain,yhat=predict(fit3)) 
t.d%>%
  ggplot(.,aes(x=x))+
  geom_point(aes(y=y),size=2)+
  geom_point(aes(y=yhat),col="red",pch=2,size=3,stroke=1.5)+
  ...
  xlab("X1")+ylab("Y")
```
With a total of 5 covariates the fitted values get closer to the data points. This means the fitted model also captures the deviations from the truth (blue line).

```{r}
fit5 <- lm(y~0+X1+X2+X3+X4+X5,data=dtrain)
```

```{r, echo=FALSE}
t.d <-data.frame(x=xtrain[,1],y=ytrain,yhat=predict(fit5)) 
t.d%>%
  ggplot(.,aes(x=x))+
  geom_point(aes(y=y),size=2)+
  geom_point(aes(y=yhat),col="red",pch=2,size=3,stroke=1.5)+
...
fit9 <- lm(y~-1+.,data=dtrain)
```

The fitted values perfectly match the data.

```{r, echo=FALSE}
t.d <-data.frame(x=xtrain[,1],y=ytrain,yhat=predict(fit9)) 
t.d%>%
  ggplot(.,aes(x=x))+
  geom_point(aes(y=y),size=2)+
  geom_point(aes(y=yhat),col="red",pch=2,size=3,stroke=1.5)+
