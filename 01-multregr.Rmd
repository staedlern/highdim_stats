...
The following table shows the regression coefficients for the different models. The larger the $p$, the bigger the discrepancy to the true coefficients.

```{r echo=FALSE}
t.d <- data.frame(matrix(0,nrow=16,ncol=4))
t.d[1:2,1] <- coef(fit1)
t.d[1:5,2] <- coef(fit4)
t.d[1:9,3] <- coef(fit8)
...
```{r echo=FALSE}
ave.deviation <- apply(t.d,2,FUN=function(x)
{
