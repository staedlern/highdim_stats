...
Next, we plot the fitted values against $X_1$ and we add the reference line representing the “true” relationship. 
...
From the `summary` we observe that the regression coefficients are clearly *over-estimated* (note the true coefficients $\beta = (2, 0, \ldots, 0)$). The model "overfits" the data. 
...
In this simulation exercise we illustrated that overfitting in high-dimensional settings becomes a real threat. In particular, if $p>n$ then it is possible to form a linear combination of the covariates that perfectly explains the response, including the noise. Further, we noted that large estimates of regression coefficients are an indication of overfitting. Finally, a useful heuristic quantity when dealing with overfitting is the *residual degrees of freedom* given by $n-{\rm df}$, where ${\rm df}$ are the degrees of freedom of the model (for a linear regression model ${\rm df}=p+1$). A rule of thumb to avoid overfitting is that the residual degrees of freedom $n-{\rm df}$ should be at least $5$.
